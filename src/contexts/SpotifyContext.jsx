...
export const SpotifyProvider = ({ children }) => {
    ...
    const [spotifyEnabled, setSpotifyEnabledState] = useState(() => {
        return getStorageItem(ENABLED_STORAGE_KEY, false);
    });

    const [playerState, setPlayerState] = useState(null);

    const setSpotifyEnabled = useCallback((value) => {
        setSpotifyEnabledState(value);
        setStorageItem(ENABLED_STORAGE_KEY, value);
    }, []);

    const updateToken = useCallback((tokenData) => {
    ...
    useEffect(() => {
        ...
        const fetchPlayer = async () => {
            try {
                const state = await request("/me/player");
